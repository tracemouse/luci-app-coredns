...
DOWNLOAD() (
	...
	# get repos info
	coredns_version=`curl -sk https://api.github.com/repos/sbwml/luci-app-coredns/releases | grep "tag_name" | grep v5 | head -n 1 | awk -F ":" '{print $2}' | sed 's/\"//g;s/,//;s/ //'`
	curl -sk --connect-timeout 10 "https://api.github.com/repos/sbwml/luci-app-coredns/releases" | grep "browser_download_url" | grep "$coredns_version" > $TMPDIR/releases.txt
	if [ $? -ne 0 ]; then
		echo -e "${RED_COLOR}Failed to get version information, Please check the network status.${RES}"
		rm -rf $TMPDIR
		exit 1
	fi
	coredns=$(cat $TMPDIR/releases.txt | grep "browser_download_url" | grep coredns_5 | grep $platform.ipk | head -1 | awk '{print $2}' | sed 's/\"//g')
	v2dat=$(cat $TMPDIR/releases.txt | grep "browser_download_url" | grep v2dat_ | grep $platform.ipk | head -1 | awk '{print $2}' | sed 's/\"//g')
	luci_app=$(cat $TMPDIR/releases.txt | grep "browser_download_url" | grep luci-app-coredns_ | head -1 | awk '{print $2}' | sed 's/\"//g')
	luci_i18n=$(cat $TMPDIR/releases.txt | grep "browser_download_url" | grep luci-i18n-coredns-zh-cn | head -1 | awk '{print $2}' | sed 's/\"//g')
	...
	# download
	echo -e "${GREEN_COLOR}Download $coredns ...${RES}"
	curl --connect-timeout 30 -m 600 -kLo "$TMPDIR/coredns_$platform.ipk" $mirror$coredns
	if [ $? -ne 0 ]; then
		echo -e "${RED_COLOR}Error! download $coredns failed.${RES}"
		rm -rf $TMPDIR
		exit 1
	fi
	...
	curl --connect-timeout 30 -m 600 -kLo "$TMPDIR/luci-app-coredns.ipk" $mirror$luci_app
	if [ $? -ne 0 ]; then
	...
	curl --connect-timeout 30 -m 600 -kLo "$TMPDIR/luci-i18n-coredns-zh-cn.ipk" $mirror$luci_i18n
	if [ $? -ne 0 ]; then
...
INSTALL() (
	...
	opkg install $TMPDIR/coredns_$platform.ipk
	opkg install $TMPDIR/luci-app-coredns.ipk
	opkg install $TMPDIR/luci-i18n-coredns-zh-cn.ipk
